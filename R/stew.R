...
# stew function for RasterStackBrickTS
          ...
          function(x, stat, d, output, pv_sig, ...) {
            ...
            if (missing(output)) output <- 'rts'
              ...
              if (!all(output %in% c('raster','rts','tau'))) {
                warning('Not all the specified items in output are recognized, default "rts" is used')
                output <- 'rts'
              }
            }
...
# stew function for SpatRasterTS
